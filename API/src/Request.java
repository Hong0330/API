...
public class Request extends Thread{
    ...
    ArrayList<ArrayList<String>> participantList = new ArrayList<ArrayList<String>>(); //참여자 소환사 정보
    ...
    public boolean callSummonerDTO(String name) throws ParseException {
    	try {   		
    		name = URLEncoder.encode(name, "UTF-8");  //한글을 인코딩하여 인식하게 함
    ...
    public boolean callLeagueEntryDTO(String id) throws ParseException {
    	try {
            ...
            if (myConnection.getResponseCode() == 200) {
                ...
                String receiveMsg = buffer.toString();
                receiveMsg = receiveMsg.replaceAll("[\\[\\]]", ""); //대괄호 삭제
                JSONParser jsonParser = new JSONParser();
                
                if("".equals(receiveMsg)) { // TFT를 한번도 플레이 하지 않을 때 
                	System.out.println("플레이 내역 없음");
                	return false;
                }               
    ...
    public void setMatch(JSONObject jsonObject) throws ParseException {
    	...
    	//participants파싱	
    	ArrayList<ParticipantDto> participants = new ArrayList<ParticipantDto>(); //최종적으로 저장할 리스트
    	...
    	for(int i = 0 ; i < participantsTmp.size() ; i++) {		
    		...
    		// companion파싱
    		CompanionDto companion = new CompanionDto(); //최종 저장 
    		...
    		// 시너지 파싱
    		ArrayList<TraitDto> traits = new ArrayList<TraitDto>(); //최종 저장
    		...
    		//유닛 파싱
    		ArrayList<UnitDto> units = new ArrayList<UnitDto>(); //최종 저장
